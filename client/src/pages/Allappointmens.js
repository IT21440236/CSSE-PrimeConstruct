...
const Allapp = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const response = await axios.get("/allAppointment", {
    ...
    const fetchAppointments = async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:8000/api/allAppointment",
          { headers: { Authorization: `Bearer ${token}` } }
  ...
  const filteredAppointments = appointments.filter((appointment) => {
    ...
    return (
      searchRegex.test(appointment.appid) || searchRegex.test(appointment.email)
    );
  });
    ...
    <>
      <div>
        ...
        <div className="d-flex justify-content-between">
          ...
          <div>
            ...
            <Link className="btn btn-outline-danger mb-2 " to="/allexp">
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        ...
        <Modal.Body>
          ...
          <p>
            <strong>Start-time</strong>: {modalData.start}
          </p>
          <p>
            <strong>End-time</strong>: {modalData.end}
          </p>
          <p>
        ...
        <Modal.Footer>
          ...
          <button
            ...
            onClick={() => modalData && deleteAppointment(modalData._id)}
          >
            Delete
          </button>
          <button
