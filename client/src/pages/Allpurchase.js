...
const Allpur = () => {
  ...
  const handleReceived = async () => {
    try {
      const response = await fetch(
        `http://localhost:8000/api/purchaseOrders/${modalData._id}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleCountClick = async () => {
    try {
      const supplierCounts = filteredPurchases.reduce((counts, purchase) => {
        const supid =
          purchase.supid && purchase.supid._id
            ? purchase.supid._id.toString()
            : "";
        if (supid) {
      ...
      Object.entries(supplierCounts).forEach(async ([supid, count]) => {
        try {
          ...
          const supplier = await fetch(
            `http://localhost:8000/api/supplier/${supid}`,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
              },
          ...
          console.log(
            `Supplier ${supid} has a count of ${count} and is rated ${rating}`
          );
          const response = await fetch(
            `http://localhost:8000/api/updateSupplierRate/${supid}`,
            {
              method: "PUT",
              headers: {
          ...
          console.log(
            `Updated supplier ${updatedSupplier.name} with new rating: ${updatedSupplier.rate}`
          );
        } catch (error) {
          console.error(error);
        }
  ...
  const handlePrint = (orderid) => {
    ...
    doc.text(
      `Supplier ID: ${
        order.supid && order.supid.supid ? order.supid.supid.toString() : ""
      }`,
      marginLeft,
      70
    );
    doc.text(
      `Requested Date: ${
        order.requestedDate ? order.requestedDate.toString() : ""
      }\n\n`,
      marginLeft,
      90
    );
    ...
    <>
      <div>
        ...
        <table className="table table-hover">
          <thead>
            <tr>
              <th scope="col" style={{ width: "15%", whiteSpace: "nowrap" }}>
                Order ID
              </th>
              <th scope="col" style={{ width: "15%", whiteSpace: "nowrap" }}>
                Sup ID
            ...
            {filteredPurchases.map((purchase) => (
              ...
              >
                ...
                <td>
                  {purchase.completed
                    ? "Done"
                    : moment(purchase.reqdate).diff(moment(), "days") >= 0
                    ? `Pending (${moment(purchase.reqdate).diff(
                        moment(),
                        "days"
                      )} days)`
                    : `Due (${Math.abs(
                        moment(purchase.reqdate).diff(moment(), "days")
                      )} days)`}
                </td>
                <td>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        ...
        <Modal.Body>
          ...
          <p>
            <strong>
              Supplier ID: {modalData.supid && modalData.supid.supid}
            </strong>
          </p>
          <p>
            <strong>
              Requested Date: {new Date(modalData.reqdate).toLocaleDateString()}
            </strong>
          </p>
          <p>
            <strong>
              Remaining Days: {moment(modalData.reqdate).diff(moment(), "days")}
            </strong>
          </p>
          <p>
            <strong>
        ...
        <Modal.Footer>
          <button
            ...
            onClick={() => handlePrint(modalData.orderid)}
          >
            Print
          </button>
          <button
            className="btn btn-danger"
            onClick={() => modalData && handleDelete(modalData._id)}
          >
            Delete
          </button>
          <button
