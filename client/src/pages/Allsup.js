...
const Allsup = () => {
    ...
    if (window.confirm("Are you sure you want to delete this supplier?")) {
      try {
        const res = await fetch(
          `http://localhost:8000/api/deleteSupplier/${id}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const filteredSuppliers = suppliers.filter((supplier) => {
    ...
    return (
      searchRegex.test(supplier.name) ||
      searchRegex.test(supplier.address) ||
      searchRegex.test(supplier.supid) ||
      searchRegex.test(supplier.email)
    );
  });
    ...
    <>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        ...
        <Modal.Body>
          ...
          <p>
            <strong>Contact Number</strong>: {modalData.mobile}
          </p>
          <p>
        ...
        <Modal.Footer>
          ...
          <button
            ...
            onClick={() => modalData && deleteSupplier(modalData._id)}
          >
            Delete
          </button>
          <button
