...
export const ViewFuelDetails = () => {
  ...
  const getdata = async (e) => {
    const res = await fetch(
      `/api/getfueldata?vehicleNo=${vehicleNo}&startDate=${startDate}&endDate=${endDate}`,
      {
        method: "GET",
        headers: {
  ...
  const handlePrintAll = () => {
    ...
    doc.text(
      `\nTotal Fuel: ${totalFuel} liter`,
      marginLeft,
      doc.autoTable.previous.finalY + 10
    );
    doc.text(
      `\n\nTotal Cost for Fuel: Rs.${totalCost}`,
      marginLeft,
      doc.autoTable.previous.finalY + 10
    );
                  ...
                  {getfueldata.map((opts, i) => {
                    return (
                      <TableRow>
                        ...
                        <TableCell className="tableCell">
                          {new Date(opts.fueldate).toLocaleDateString()}
                        </TableCell>
                      </TableRow>
                    );
                  })}
